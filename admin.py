...
def run():
    ...
    st.caption(f"Voting Members: {_inattendance_voting_members}")
    st.dataframe(df_member_attendance, hide_index=True)
    ...
    st.caption(f"Non-Voting Members, Non-Members, Staff: {len(df_nonmember_attendance)}")
    st.dataframe(df_nonmember_attendance, hide_index=True)
